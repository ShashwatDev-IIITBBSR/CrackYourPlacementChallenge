...
public:
    string shortestCommonSupersequence(string str1, string str2) 
    {
        ...
        while(i>0&&j>0)//condition 
        {
            ...
            {
                if(mat[i-1][j]>mat[i][j-1]){a.push_back(str1[i-1]);i--;}
                else{a.push_back(str2[j-1]);j--;}
            }
        }
        while(i!=0){a.push_back(str1[i-1]);i--;}
        while(j!=0){a.push_back(str2[j-1]);j--;}
        reverse(a.begin(),a.end());
        return a;
        /*while(mat[i][j]!=0)
        {
